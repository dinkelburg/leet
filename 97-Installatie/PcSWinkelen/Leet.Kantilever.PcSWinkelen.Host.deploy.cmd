...
set _DeploySetParametersFile=%RootPath%Leet.Kantilever.PcSWinkelen.Host.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%Leet.Kantilever.PcSWinkelen.Host.zip" (
echo "%RootPath%Leet.Kantilever.PcSWinkelen.Host.zip" does not exist. 
...
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Leet.Kantilever.PcSWinkelen.Host.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension
) else (
set _MSDeployCommandline="%MSDeployPath%msdeploy.exe" -source:package='%RootPath%Leet.Kantilever.PcSWinkelen.Host.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%_DeploySetParametersFile%"
)
...
:usage
echo =========================================================
if not exist "%RootPath%Leet.Kantilever.PcSWinkelen.Host.deploy-readme.txt" (
...
start notepad "%RootPath%Leet.Kantilever.PcSWinkelen.Host.deploy-readme.txt"
)
echo =========================================================
goto :eof
